...
RetailPricePlot1 <- ggplotly(
 ...
 labs(x = "Months", y = "Average Retail Price", title = "Average Retail Price of Computer in 2018", 
...
RetailPricePlot2 <- ggplotly(
 ...
 geom_line(size = 0.4, 
 colour = "#1c6155") + geom_point() +
 labs(x = "Weeks", y = "Average Retail Price", title = "Average Retail Price of Computer in 2018", 
...
> Those Plot shows different aggregations levels, can be used depending on the analysis we want, thus the granularity need. End of the weekdays is generally having higher retail prices, such as the year starting with month period from May to December. 
...
> The following barplots show two ways of analyzing the stores sales results: by the number of transactions or the sales revenues they each generated during 2018. The Store SW1P 3AU sold the most and with the highest revenues. 
...
```{r 11134,echo=FALSE,warning=FALSE,message=FALSE,results='hide'}


{plot(x=Data_Client_Coordinates$customer.X, y=Data_Client_Coordinates$customer.Y, main="Distance Between Customers and Stores",xlab="Longitude",ylab="Latitude",cex=0.5,col="#1c6155", pch=20)+ points(Data_Stores_Coordinates$store.X, Data_Stores_Coordinates$store.Y,cex=1, col="#3d71a8", pch=17) + mtext("Blue = Stores , Green = Customers", side=4)}
